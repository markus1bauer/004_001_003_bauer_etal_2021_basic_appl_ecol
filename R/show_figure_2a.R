...
flowers <- rename(flowers, "predicted" = "prop.flow.spec", "x" = "month", "group" = "treatment")

pd <- position_dodge(.6)

(graph_a <- ggplot() +
    geom_quasirandom(data = flowers, 
                     aes(x = x, predicted, shape = group),
    ...
    annotate("text", 
             ...
             x = c(1, 2, 3, 4, 5), 
             y = c(32, 32, 32, 32, 32)) +
    scale_y_continuous(limits = c(-.1, 32), breaks = seq(-100, 400, 5)) +
